...
extern "C"
{
	...
	__declspec(dllexport) void b2WheelJoint_SetMotorSpeed(void* wheel, double speed) {
	  ((b2WheelJoint*)wheel)->SetMotorSpeed(speed);
	}

	__declspec(dllexport) void b2WheelJoint_SetSpringFrequencyHz(void* wheel, double frequency) {
	  ((b2WheelJoint*)wheel)->SetSpringFrequencyHz(frequency);
	}

	__declspec(dllexport) void* b2WheelJointDef_Create(
		void* world,
		// joint def
	...
	__declspec(dllexport) void* b2WheelJointDef_InitializeAndCreate(
		void* world,
		// initialize args
